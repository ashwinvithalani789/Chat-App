...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	[DownloadManager clearManualImage:dbmessage.picture];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	NSInteger network = [FUser networkImage];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if ((network == NETWORK_MANUAL) || ((network == NETWORK_WIFI) && ([Connection isReachableViaWiFi] == NO)))
	{
		rcmessage.status = STATUS_MANUAL;
	}
	else [self downloadPictureMedia:rcmessage dbmessage:dbmessage tableView:tableView];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	[DownloadManager clearManualVideo:dbmessage.video];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	NSInteger network = [FUser networkVideo];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if ((network == NETWORK_MANUAL) || ((network == NETWORK_WIFI) && ([Connection isReachableViaWiFi] == NO)))
	{
		rcmessage.status = STATUS_MANUAL;
	}
	else [self downloadVideoMedia:rcmessage dbmessage:dbmessage tableView:tableView];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	[DownloadManager clearManualAudio:dbmessage.audio];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	NSInteger network = [FUser networkAudio];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if ((network == NETWORK_MANUAL) || ((network == NETWORK_WIFI) && ([Connection isReachableViaWiFi] == NO)))
	{
		rcmessage.status = STATUS_MANUAL;
	}
	else [self downloadAudioMedia:rcmessage dbmessage:dbmessage tableView:tableView];
}

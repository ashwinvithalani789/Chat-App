...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
		FUser *user = [FUser currentUser];
		user[FUSER_LASTACTIVE] = [FIRServerValue timestamp];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
		FUser *user = [FUser currentUser];
		user[FUSER_LASTTERMINATE] = [FIRServerValue timestamp];
		[user saveInBackground:^(NSError *error)
		{
			if (fetch) [user fetchInBackground];
		}];
	}
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	if ([Blocker isBlocker:dbuser.objectId] == NO)
	{
		if (dbuser.lastActive < dbuser.lastTerminate)
		{
			return [NSString stringWithFormat:@"last active: %@", TimeElapsed(dbuser.lastTerminate)];
		}
		else return @"online now";
	}
	else return nil;
}

...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if ([text length] != 0)
	{
		...
		predicate = [NSPredicate predicateWithFormat:format, text, text, text];
	}
	else predicate = [NSPredicate predicateWithFormat:@"isDeleted == NO"];
	//---------------------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	labelTitle.text = [NSString stringWithFormat:@"(%ld contacts)", (long) [dbcontacts count]];
	//---------------------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	for (DBContact *dbcontact in dbcontacts)
	{
		...
		if (dbuser != nil)
		{
			...
			attributeSet.title = dbcontact.fullname;
			attributeSet.displayName = dbcontact.fullname;
			attributeSet.contentDescription = dbuser.country;
			attributeSet.keywords = @[dbcontact.firstname, dbcontact.lastname, dbuser.country];
			...
			[items addObject:[[CSSearchableItem alloc] initWithUniqueIdentifier:dbuser.objectId domainIdentifier:bundleId attributeSet:attributeSet]];
		}
	}
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[[CSSearchableIndex defaultSearchableIndex] deleteSearchableItemsWithDomainIdentifiers:@[bundleId] completionHandler:^(NSError *error)
	{
		if (error == nil)
		{
